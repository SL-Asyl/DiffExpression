...
for (file_path in raw_files) {
  ...
  tb_raw <- tryCatch({
    ...
    message("  Ошибка при чтении файла ", file_path, " Пропускаем."); next
  })
  ...
  if (nrow(tb) == 0) { message("  Файл пустой после очистки NA. Пропускаем."); next }
  ...
  if (nrow(tb) == 0) { message("  Не удалось сопоставить ENTREZ ID для генов. Пропускаем."); next }
